...
//		페이징 처리
	...
	public String inquiryInsertPost(@ModelAttribute InquiryVO iqVo ,HttpSession session) {

		//파일 업로드
		String fileGrNum= fileController.fileUpload(iqVo.getFileList()); 
	  ...
	  //상세내역
	   ...
	   public String inquDetail(@RequestParam String artcNum, Model model) {
	  	...
	  	if(iv.getFileGrNum() != null) { // 파일이 들어 있을 경우
			List<AttachFileVO> fileList = fileController.fileList(iv.getFileGrNum());
	 ...
	 //수정
		...
		public String inquiryUpdate(@ModelAttribute InquiryVO inquiryVo, Model model
			...
			if (fileCheck != null) {
				String fileGrNum = fileController.fileUpload(inquiryVo.getFileList());
